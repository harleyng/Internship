...
import ReportForm from "../../../pages/student/evaluation/ReportForm";
...
const ReportEvaluation = ({
  ...
  useEffect(() => {
    ...
    setTotal((prevData) => {
      return { ...prevData, report: evaluation.report?.totalScore };
    });
  }, [profile, evaluation]);
    ...
    if (e.target.name.includes("review")) {
      ...
      setEvaluationData({
        ...
        report: {
          ...
          review: { ...evaluationData.report?.review, [key]: numberValue },
        },
      });
      if (e.target.value) {
        console.log(total);
        ...
        setTotal((preData) => {
          console.log(preData);
          return { ...preData, report: preData.report + numberValue };
        });
      } else {
        console.log(e.target.value);
        const name = evaluationData.report.review[key];
        setTotal((preData) => {
          return { ...preData, report: preData.report - name };
        });
      }
    } else if (e.target.name === "ratings") {
      setEvaluationData({
        ...evaluationData,
        report: { ...evaluationData.report, [e.target.name]: numberValue },
      });
    } else if (e.target.name) {
      setEvaluationData({
        ...evaluationData,
        report: { ...evaluationData.report, [e.target.name]: e.target.value },
      });
    }
  ...
  return (
    <>
      <ReportForm
        ...
        sendEvaluationForm={sendEvaluationForm}
        reportData={evaluationData.report}
        total={total}
      />
    </>
  );
